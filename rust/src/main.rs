// Original example code by @jimblandy from O'Reily's Programming Rust
// Modifications by @dodrian
// MIT License
...
fn main() {
    ...
    if args.len() < 5 {
        writeln!(std::io::stderr(),
                 "Usage: mandelbrot FILE PIXELS UPPERLEFT LOWERRIGHT [MAX_THREADS [OMIT_DRAWING]]")
            .unwrap();
        writeln!(std::io::stderr(),
                 "Example: {} mandel.png 1000x750 -1.20,0.35 -1,0.20 4 1",
                 args[0])
            .unwrap();
        std::process::exit(1);
    }
    ...
    let threads = match args.len() >= 6 {
        true => usize::from_str(&args[5]).expect("error parsing number of threads"),
        false => 8
    };
